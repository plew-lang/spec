...
**トレイト名は ambient な名前ではありません。** ただし `a + b`・`for`・`xs[i]`・`x?`・`a == b`・`xs.sort()` のように**トレイトに裏打ちされた機能を使う**ときは、トレイト名自体を解決しないため import は不要です：
...
トレイト名を書く必要があるのは `where T: Add`・`any Iterator[Item=I32]`・`impl MyVec as Add` のように**自分でジェネリック/トレイト/impl を書く＝定義地点**だけで、そこでは他の名前と同じく `import` します（組み込みの演算子トレイトも標準ライブラリから `import`）。`xs.sort()` を**呼ぶ**側には波及しません ── `sort` 側の `where T: Ord` は定義地点に書いてあり、呼び出しは型に対して解決されるからです。

同じ境界は `#` にも適用します。`a#P.foo()`／`A#P` は trait `P` を**名前として選択**するので `P` の import が必要です。`a.foo()`／`a + b` のように trait を暗黙に経由する操作は import 不要ですが、`impl`・`where`・`any`・`#Trait`・`T#Trait.Item` のように trait 名を直接書く操作は import 必須です。`#Ext` も同様に extension 名を選ぶので、その extension を import で束縛している必要があります。
...
#### run・check・build の検査と実行

コマンドは**検査規則を共有し、検査を要求する範囲とタイミングを分けます**。

| コマンド | 契約 |
|---|---|
| `plew run` | 必要になった宣言・関数とその検査依存を順次検査し、必要なコードをJITして実行する。全体の事前検査は既定で行わず、未検査の本体が残ることを許す |
| `plew check` | 対象プロジェクトの全体を静的に検査する。未使用本体・generic本体も対象とし、アプリケーションを実行せず、機械語を生成しない |
| `plew build` | 対象全体を静的に検査し、LLVMで最適化されたバイナリを生成する。検査範囲とコード生成の到達性は別であり、未使用コードの除去を許す |

対象全体は、選択したプロジェクト・ビルド対象・設定に属するソースとその依存です。無関係なファイルや別ターゲットの設定を無条件に混ぜません。全体の事前検査を求める利用者は、同じ対象・設定に対して `plew check && plew run …` を使えます。事前検査を既定にする変更や `--no-check` オプションは、この仕様には含めません。`run` の成功は対象全体の検査成功を意味しません。

**遅延検査の基本単位は関数本体**です。実行する本体は、その実行開始前に、未実行分岐を含めて型・所有権・借用・制御フローの検査を済ませます。行や実行済みの経路だけを検査する方式にはしません。closureや初期化子などの本体にも同じ原則を適用します。generic本体は、要求された時点で宣言のboundに基づく抽象検査を行い、具体instanceで偶然通ることを根拠に受理しません（[genericの検査](../02-type-system/06-generics.md#ジェネリック本体の型検査宣言時abstract)）。

「必要な範囲」は、実際に呼ばれた関数だけではありません。名前解決、型・レイアウト、可視性、implのcoherence、関数間のeffect・借用・共有解析など、その実行を正しく検査・生成するための依存範囲を含みます。解析の性質によっては起動前に広い範囲の検査が必要になります。未知のeffectを無害とみなす、必要な静的検査を省くなどの弱化は認めません。機械語生成の遅延と検査の遅延は独立です。

**遅延するのは検査・コード生成であり、言語の評価順序ではありません。** トップレベル値は[通常の初期化規則](#トップレベル初期化と実行順序)どおり `main` 前にすべて初期化します。ARC・CoW・unique・通常の破棄順序もコマンド間で変えません。

`run` 中に遅延検査またはコンパイルが失敗した場合は、該当本体の実行を開始せず診断を報告し、アプリを停止状態にします。独立したコマンド実行は失敗として終了し、常駐する開発ホストは次の明示的なRestartを受け付けられます。失敗までに生じたI/Oなどの副作用は取り消しません。停止時にユーザーの全 `deinit` の完走を保証するものでもありません。未検査の本体に誤りがあるプログラムが途中まで動くことは `run` の契約であり、`check`・`build` の全体検査とは区別します。

一回の実行環境は同じソース版・依存・設定に対応します。遅延読取り時に保存された別版を混入させてはならず、編集の反映は明示的なRestartで行います。検査・生成結果のキャッシュを再利用する場合も、入力・依存・設定・コンパイラの整合性を確認します。`check` 後の入力変更に対し、古い検査結果を新しい入力の保証として扱いません。再起動ではアプリ状態を作り直し、状態を保持したコード差し替えは行いません。
...
`extern(c)` は外部コードを利用する責任境界です。コンパイラは、外部実装の読み書きが不明という理由だけで、借用中の外部関数呼び出しを拒否しません。FFI利用側は、外部コードへ渡すポインタの有効期間、読み書きの権限、所有権移譲、保持期間を確認し、Plewの `borrow` / `inout` / `move` / 破棄の規則を外部コードが破らないことを保証します。借用規則は境界を越えても有効であり、違反を必ずコンパイルエラーやpanicとして検出する保証はありません。安全なPlew APIで包む場合、この責任をラッパの実装者が引き受けます。

Plew内の型・所有権・借用検査は通常どおり行います。外部呼び出しを含む関数全体を検査対象外にはしません。将来CからPlewへのcallbackを導入する場合も、再入によって生きている借用が消えることはなく、Plew側の処理はPlewの規則に従います。この責任規則だけでcallback/exportの未定仕様を追加するものではありません。

外部呼び出しを受理することは、副作用がないとの宣言ではありません。最適化は未知の外部副作用を保守的に扱い、根拠なく呼び出しを消したり、外部から変更され得るメモリの読み書きを呼び出しの前後へ移動したりしません。外部effect注釈や新しい `unsafe` 構文は、この境界の利用条件にしません。`extern(plewIntrinsic)` はコンパイラが定義する床であり、その正しさを利用者責任へ移すものではありません。
