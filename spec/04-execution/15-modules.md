...
トップレベル値は、可変性にかかわらず**プロセスの root event loop に属する状態**です。[`spawn`](14-concurrency.md) ごとのコピーや再初期化は行いません。spawn 本体はトップレベル/`assoc` 値へ直接にも名前付き関数経由にもアクセスできず、必要な値は spawn 境界でローカル値として明示的に渡します。名前付き関数そのものは禁止せず、引数とローカル値だけで完結する関数は spawn から呼べます（→ [spawn からのトップレベルアクセス](14-concurrency.md#spawn-からのトップレベルアクセス)）。
...
初期化は**プロセス全体で一度だけ**です。後から起動した spawn スレッドは独自のトップレベル状態を持たず、初期化子も再実行しません。spawn からトップレベル値へ到達できないため、初期化回数と共有範囲が値の型や `val`/`mut val` によって暗黙に変わることもありません。
