...
> 命名規則: ディレクティブ名は `macro Foo as Derive` / `macro Foo as ParameterizedDerive` が付いた**既存宣言**の名前で、組み込み・ユーザー定義を問わず **PascalCase** です（`Eq` / `Hash` / `Encode` / `Decode` / `All`）。一方、ディレクティブが**生成する**メンバ（例: `Enum.all()`）は通常の関数・変数なので camelCase です。
...
`macro X as ...` は `X` の所有モジュール内でだけ宣言できる（v1）。第三者が外部宣言へ勝手に同名の derive provider を後付けすることはできない。公開 API として外部から使わせる derive は、通常の top-level 宣言と同じく `export macro X as ...` で公開する。

マクロは生成メソッド（要求＝`derive(input: TopItemAst) -> String`）を提供し、**注釈対象項の構文木を受け取り生成 Plew ソースを `String` で返す**。`Derive` 系インターフェースと `AST` 型は**構文ライブラリ**（当面 `@Plew/Syntax`・将来は外部共有パッケージ＝後述）が提供します。**入力型 = `TopItemAst`**（注釈対象のトップレベル項の値ツリー＝`Decl`〔struct/enum/fn〕・`Impl`・`Trait`・… のタグ付き union）。マクロは `match` で対象の種別に分岐する＝対象が何かを**明示**して扱う（struct 専用 derive は `Decl` だけ扱い他を弾く）。これは「コンパイラとマクロが唯一の同一 AST を読む（1 AST 原則）」の帰結で、マクロ専用の縮小 AST を作らない。
...
export macro Builder as ParameterizedDerive {
...
組み込み（`Eq`/`Hash` …）も理想は同じ macro 宣言。だが bootstrap・性能・基盤未整備のため、**当面はコンパイラ特権の合成のまま**据え置き、API が固まってからユーザーマクロと同じ仕組みへ移行（dogfood）する。`Ord` は derive を持たず、順序が要る型は `impl T as Ord` を手書きする。

## 確定した骨格（まとめ）

- **derive 可能性＝既存宣言に `macro X as Derive` / `macro X as ParameterizedDerive` が付いていること**。**設定なし→`Derive`（`assoc fn derive(input: AST) -> String`・bare `@[X]`）／設定あり→`ParameterizedDerive`（`fn derive`・self=設定 struct・`@[X(args)]`）**。`()` 規約で構文が機構を表す（bare=assoc／括弧=構築）。+**提供 `deriveFromSource`**（構文ライブラリ＝String→AST 変換＋委譲）。
...
- 組み込み Eq/Hash は当面コンパイラ特権・将来 dogfood。Ord は derive 不可。
