...
`plew gen` はマクロ呼び出しを入口とする別のプログラムを構成します。注釈対象は構文 AST として渡し、アプリケーションの `main` や、生成に必要でない本体・初期化子を実行しません。同一ファイルに同居していることだけでは、生成用プログラムの実行対象になりません。

生成用プログラムに含めるトップレベル／`assoc` 値は、マクロとその呼び出し先が参照する値、およびそれらの初期化に推移的に必要な値です。初期化子から呼ぶ関数も依存先に含めます。この集合に対して、[通常のトップレベル初期化規則](15-modules.md#トップレベル初期化と実行順序)を適用し、マクロ本体の実行前に初期化を完了します。

参照されない値は、初期化に副作用があっても生成用プログラムへ含めません。例えば、未参照の値の初期化子が別の global を更新していても、その副作用だけを理由に保持しません。マクロに必要な登録・準備処理には、マクロ側から明示的に依存させます。これは生成用プログラムを構成する際のツリーシェイク規則であり、通常のアプリケーションで未使用のトップレベル値も eager に初期化する規則は変わりません。
