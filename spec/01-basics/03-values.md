...
`inout` は[排他](#アクセスモードborrow--inout--move)です。1 つの呼び出しで**2 つ以上の `inout` 位置（レシーバ／引数）が同じ場所を指す**と排他の前提が破れます（get-modify-set 実装では独立コピーの書き戻しが衝突して片方の変更が静かに消え〔last-write-wins〕、in-place 実装では呼び出し先から同じ場所が 2 つの名前で見える）。これを**プログラムエラー**とし、重なったときの挙動は**保証しません**。検査は三分します：

- **静的に重なりが確定する** — 構文的に同一の場所（`f(a: inout x, b: inout x)`・`x.m(arg: inout x)`・添字式まで一致する `arr[i]` 同士）、および**一方が他方を包含する place path**（`a.merge(inout a.field)` の `a` と `a.field`・`arr` と `arr[i]`）→ **コンパイルエラー**。実行すれば必ず重なるものを実行時まで黙って通す理由はありません。
- **静的に distinct と証明できる** — 別々の値型変数を root とする place 同士（値意味論ではエイリアスし得ない）・同一 struct の別フィールド等 → **OK**（検査なし）。
- **どちらでもない** — 同一コンテナで添字の一致が実行時にしか分からない（`arr[i]` と `arr[j]`）、`Ref` 経由で同一セルを指し得る（`r1->v` と `r2->v`）など → **呼び出し直前に place の一致をランタイム検査し、一致なら panic**（多重 `inout` の呼び出し位置に限った狭い検査）。検査は place を求める際に**評価済みの値だけ**で行い、引数の部分式を再評価しません（副作用を二重に発火させない）。
