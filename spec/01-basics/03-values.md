...
- **式が渡す所有権**：関数結果・構築結果と `move value` は、受け手へ所有権を渡す値です。前者は新しい結果、後者は既存placeからの移譲という出どころだけが異なります。束縛・消費引数・消費enum matchへ移譲済みの一時値を元の場所から再び破棄しません。以下の一時値の寿命規則は、その値を所有したままの場合に適用します。
