...
名前付き関数はキャプチャを持たないため、関数値としての `sendable fn` 修飾は付きません。ただし名前付き関数を経由すれば検査を回避できるわけではありません。`spawn` 本体または `sendable fn` から名前付き関数を呼ぶと、コンパイラはその呼び先を推移的に検査します。`sendable fn` は将来どの実スレッドへ送られても成立する型保証なので、**リテラルの宣言地点自体が検査 root** です。実際に spawn へ渡された時点まで検査を遅らせません。
