...
名前付き関数はキャプチャを持たないため、関数値としての `sendable fn` 修飾は付きません。ただし名前付き関数を経由すれば検査を回避できるわけではありません。`spawn` 本体または `sendable fn` から名前付き関数を呼ぶと、コンパイラはその呼び先を推移的に検査します。
...
トップレベル/`assoc` 値へ到達せず、境界を越えた引数と戻り値が sendable である名前付き関数は `spawn` から普通に呼べます。spawn スレッド内で新しく作り、そのスレッド内だけで使う `Ref` などは構いません。関数宣言ごとに日常的な修飾を要求するのではなく、明示された `spawn` 境界を根として whole-program の呼び出しグラフを検査します。エラーは境界側の呼び出し位置に出し、そこから禁止された値までの関数列を併記します（→ [spawn からのトップレベルアクセス](../04-execution/14-concurrency.md#spawn-からのトップレベルアクセス)）。
