...
- **各デフォルト式は独立した完全式**です。正常に評価を終えたとき、引数となる結果値を呼び出し側へ渡し、式の内部でだけ使った一時値を[完全式の破棄規則](03-values.md#deinit)に従って破棄してから、次のパラメータの評価へ進みます。最後のパラメータなら、その破棄後に呼び出し先の本体へ入ります。**結果値そのものをこの時点で破棄するわけではありません**。結果値は通常の引数と同じ所有権・借用規則に従い、たとえば一時的な `unique` 値を `borrow` 引数へ渡した場合は、呼び出し側の完全式が終わるまで所有者を保持します。明示引数の寿命規則は変更しません。
...
たとえば `Resource` が `unique` 型で、`inspect(item: borrow Resource) -> Bool` が資源を借りて判定する場合：

```plew
fn consume(flag: Bool = inspect(item: <Resource />)) { ... }
consume()
```

評価順は「`Resource` の生成 → `inspect` → 一時的な `Resource` の `deinit` → `consume` 本体」です。結果の `Bool` だけが呼び出し先へ渡ります。一方、`item: borrow Resource = <Resource />` なら、`Resource` 自体が結果値なので、本体での借用が終わる前に破棄されることはありません。いずれも正常評価時の規則であり、panic／プロセス終了での破棄保証は通常の規則と同じです。
...
**名前付き関数は第一級の値ではありません。** 名前だけを `fn(...)` / `sendable fn(...)` 型の変数へ代入したり、関数値として引数へ渡したりはできず、値として存在する関数は明示的なクロージャリテラルだけです。名前付き関数を関数値から呼びたい場合は `fn(...) { foo(...) }` または `sendable fn(...) { foo(...) }` と明示的に包みます。したがって名前付き関数宣言へ `sendable` や `concurrent` の修飾は付けません。

コンパイラは各名前付き関数を解析するときに、可変／nonsendable なトップレベル状態への到達、引数・戻り値の sendability、呼び先、暗黙実行を含む effect summary を内部表現へ記録します。関数本体の解析を spawn から到達するまで遅らせるのではなく、`spawn` 本体または `sendable fn` から名前付き関数呼び出しへ到達したときに保存済み summary を推移的に照合します。`sendable fn` は将来どの実スレッドへ送られても成立する型保証なので、**リテラルの宣言地点自体が検査 root** です。実際に spawn へ渡された時点まで検査を遅らせません。
...
spawn {
    ...
    // エラー：呼び先がトップレベル変数 requestCount へ到達する
}
```

可変／nonsendable なトップレベル/`assoc` 値へ到達せず、境界を越えた引数と戻り値が sendable である名前付き関数は `spawn` から普通に呼べます。sendable な不変トップレベル/`assoc val` の読み取りは許されます。spawn スレッド内で新しく作り、そのスレッド内だけで使う `Ref` なども構いません。関数宣言ごとに日常的な修飾を要求するのではなく、明示された `spawn` と `sendable fn` 宣言を根として whole-program の実行依存を検査します。エラーは `spawn` なら境界側の呼び出し位置、`sendable fn` ならリテラルの宣言地点に出し、そこから禁止された値までの関数列を併記します（→ [spawn からのトップレベルアクセス](../04-execution/14-concurrency.md#spawn-からのトップレベルアクセス)）。
...
- **閉包がスコープの外へ脱出**（返す・格納する）しても、キャプチャした変数は閉包が生きている間存続します。だから状態を持つ閉包に `MutableRef` は要りません。これはクロージャ環境のレキシカルな共有可変であり、`MutableRef` のように独立した共有可変オブジェクトを明示構築する機構とは別です：
...
- **単一スレッドだからデータ競合は起きない**：このイベントループモデルでは共有された可変変数へのスレッド間データ競合は起きず、論理的なインタリーブ（再入）だけです（JS と同じ）。ただし [`inout` の memory exclusivity](03-values.md#呼び出し中の同時アクセス防止) は別規則です。`inout` 中に同じ storage を closure capture 経由で読む／書く形は、単一スレッドでも排他違反として拒否または runtime panic になります。
