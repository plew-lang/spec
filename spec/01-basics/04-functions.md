...
名前付き関数はキャプチャを持たないため、関数値としての `sendable fn` 修飾は付きません。**`concurrent fn` のような宣言も持ちません。** コンパイラは各関数を解析するときに、可変／nonsendable なトップレベル状態への到達、引数・戻り値の sendability、呼び先、暗黙実行を含む effect summary を内部表現へ記録します。関数本体の解析を spawn から到達するまで遅らせるのではなく、`spawn` 本体または `sendable fn` から名前付き関数へ到達したときに保存済み summary を推移的に照合します。`sendable fn` は将来どの実スレッドへ送られても成立する型保証なので、**リテラルの宣言地点自体が検査 root** です。実際に spawn へ渡された時点まで検査を遅らせません。
