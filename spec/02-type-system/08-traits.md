...
- **トレイト間準拠 `impl B as A` も同じ**：`B` の全準拠型を別トレイト `A` へ準拠させる宣言は、**主語トレイト `B` を所有するモジュール内だけ**ベアに書ける（準拠は `B` 準拠型のベア表面に自動随伴）。`A` を所有するだけでは不可で、`A` owner が外部の `B` を `A` へ準拠させたい場合は `extension` 経由で `#Ext` opt-in する。複数経路で同じトレイトに届く衝突は `#` で経路を選び、active なトレイト間準拠グラフの循環は reject する（→ [拡張：トレイト間準拠の衝突](09-extensions.md#トレイト間準拠の衝突は-でパスを選ぶ)、[トレイト間準拠の循環](09-extensions.md#トレイト間準拠の循環はエラー)）。頭なし型変数 `impl[T] T as A where T: B` は禁止（錨が無く列挙不能）で、主語トレイト `B` を錨にした `impl B as A` で書く。
