...
外部型を自分のトレイトに準拠させる際、トレイトの要求名が型の既存メソッドと衝突しても（シグネチャ不一致で `via` 束縛できなくても）拡張内で定義でき、`#Ext` で区別できます ── これを禁止すると当該準拠が不可能になるため、同名追加を許容します。トレイト準拠は `via` で実体メンバに束ねます（[トレイト準拠と via](08-traits.md) 参照）。拡張の impl からのアクセスも定義モジュールで判定します。同じモジュールの型なら非公開メンバも見え、別モジュールの型なら `pub` / `pub(get)` の範囲だけを利用できます（→ [メンバの可視性](05-structs-enums.md#メンバの可視性)）。
...
- 可視性は**字句的**で `self` の型に依らない。対象型と同じ定義モジュールのコードなら非公開メンバも見え、別モジュールなら `pub` / `pub(get)` の範囲だけが見え、`self#!Bar` は解決先を変えるだけで可視性は広げない（`self#!Bar.m()` も元メンバの通常の可視性に従う）。
