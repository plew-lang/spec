...
`impl` の**主語は型でもトレイトでもよい**：`impl Type { … }` は型に、`impl Trait { … }` はそのトレイトの全準拠型に（提供メソッド）、`impl B as A { … }` はトレイト `B` の全準拠型を別トレイト `A` へ準拠させる（トレイト間準拠）。**トレイト自身の提供メソッドも、トレイト間準拠も、ベアに書けるのはそのトレイトを所有するモジュールだけ**（`pub impl Trait` は外部 surface、修飾なし `impl Trait` は所有モジュール内 helper。`impl B as A` は `A`／`B` を所有するモジュール内ならベア可・型の inherent `impl Type` と同じ所有者コヒーレンス・→ [トレイト](08-traits.md)）。拡張内の `impl Trait`／`impl B as A` は**第三者が（所有者でなくても）足す追加分**で、`#Ext` で opt-in します。1 つの拡張に複数の `impl` を束ねられ（型・トレイト混在可）、**拡張の名前からは中身を推定できません**（後述の à la carte 適用）。
...
**いずれもトレイトを所有するモジュール内ならベアに書けます**（`pub impl Trait` の提供メソッドは準拠型の外部ベア表面に自動で生え、修飾なし `impl Trait` は所有モジュール内限定の helper になる。`impl B as A` は `B` の全準拠型が `A` の準拠をベアに得る）。**第三者**（トレイト所有者でない側）が同じことをするのは `extension` 経由で、使用箇所で `#Ext` を明示して opt-in します。拡張に書くのはその追加分です。トレイト間準拠の衝突解決は[後述](#トレイト間準拠の衝突は-でパスを選ぶ)。
...
- **衝突＝宣言地点でエラー**：型自身のメソッド・準拠トレイトの公開提供メソッド・既定にした拡張が同セレクタ・同シグネチャで衝突したら、その型の定義地点でコンパイルエラー（引数型が違えばオーバーロード共存）。片方を既定にしない（`#Ext` 明示）で解決する。
- **局所的に剥がす**：使用箇所で `a#!Ext` と書けば、その式に限り既定拡張のビューを外せる（[super 呼び `self#!Bar`](#拡張メソッド内の-self-の型) と同じ value／型レベルの strip）。

> トレイト自身の公開提供メソッドはこの仕組みではなく**準拠で自動随伴**します（→ [トレイト](08-traits.md)）。`defaultExtension` は**拡張専用**で、トレイトには付けません。`#!` でトレイトの既定拡張を剥がすような記法も（その機構が無いので）ありません。
