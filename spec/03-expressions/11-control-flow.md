...
- **match のアクセスモード**：コピー可能な storage place は裸の `match x` と書けます。`unique` な storage place は裸の `match x` では消費せず、v1 ではコンパイルエラーです。所有権を渡して分解したい場合は `match move x` と書きます。名前の無い `unique` 値（関数結果・構築結果など）は評価結果として所有権を渡すため裸で書けます。enum では `match move x` と同じ consuming match となります。struct の copyable field 読み取りでは元の値全体を保持し、match 文なら文終端、値位置 match なら式終端で破棄します。struct の field partial move は許可しません。
- **consuming match**：明示 `move` または名前の無いunique値から所有権を受け取るenum matchは、subject全体をconsumeします。`match move x` は元の `x` を消費済みにし、`match makeEnum()` は関数結果を一度評価してその所有権を受け取ります。enum の arm 判定は subject を破壊せずに行い、最終 arm が決まった後で active payload を pattern に従って束縛または破棄します。したがって refutable pattern の試行途中で subject を部分破壊して次 arm を試せなくなることはありません。match 後、移譲元の所有者に破棄責任は残りません。明示moveした元のplaceはmovedであり使えません。
- **`match borrow` / `match inout` は v1 では持ちません**。有用にするには arm scope を持つ借用束縛・inout 束縛が必要ですが、これは借用変数とライフタイム管理の導入を伴うため将来機能とします。
- **aggregate partial move は v1 では持ちません**。`move p` のように値全体を移すことはできますが、`move p.field` やパターンで一部のフィールドだけを move out し、残りの aggregate を生かす形は reject します。`match move` の enum payload 束縛は例外ではなく、subject 全体を consume した後に active payload をそのarm内の束縛へ移すだけです。`deinit` 本体は全フィールドが有効な値を前提に走るため、`unique struct` の field partial move とは相性が悪く、deinit の無い aggregate でも move path / drop flag / partial-initialization state を v1 に持ち込まないため同じく reject します。将来必要になった場合は、`swap` / `replace` / `Optional.take()` のような明示 API で container 全体の有効性を保ったまま値を取り出す形で追加します。
...
- **`unique` payload の `_`**：consuming match で active payload の `unique` component を `_` で受けた場合、その component は pattern commit 時に破棄されます。複数の component は payload 宣言順に処理し、`val` で束縛した component は通常のローカル束縛として後で破棄されます。
